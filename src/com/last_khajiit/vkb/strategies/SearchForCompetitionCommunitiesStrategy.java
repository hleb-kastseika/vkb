...
public class SearchForCompetitionCommunitiesStrategy extends BehaviourStrategy{
	...
	private void searchCommunities() throws JSONException{
		...
		competitionkeyWords.addAll(Arrays.asList(propertyManager.getProp(Properties.COMMUNITIES_SEARCH_WORDS).split(COMMA_SEPARATOR)));
		...
		for(String keyWord: competitionkeyWords){
			...
			if(searchedCommunities.contains(RESPONSE_NODE) &&searchedCommunities.contains(ITEMS_NODE)){
				JSONArray communitiesList = new JSONObject(searchedCommunities).getJSONObject(RESPONSE_NODE).getJSONArray(ITEMS_NODE);
				for(int i=0; i< communitiesList.length(); i++){
					...
					if((Integer)community.get(IS_CLOSED_NODE) != 2
							...
							|| community.getString(TYPE_PROPERTY).equals(PAGE)) && (Integer)community.get(IS_MEMBER_NODE) == 0){
						//validate that community name don't contains blocked words
						if(!BlackWordsDetector.getInstance().isContainedInBlackWordsList(community.getString(NAME_PROPERTY))){					
							String wallPosts = TextUtils.cp1251ToUTF(apiClient.getWallPosts(COMMUNITY_MAREKER+community.get(ID_NODE)));
							...
							BlackWordsDetector.getInstance().addToBlockedCommunities(community.get(ID_PROPERTY).toString());
							Logger.info("## community with ID = ["+community.getLong(ID_PROPERTY)+"] is added to communities black list");
						}
					}else{
						Logger.info("## community with ID = ["+community.getLong(ID_PROPERTY)+"] is already joined");
					}			
				}
			}			
