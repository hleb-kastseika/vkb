...
public class CheckCurrentCommunitiesWallsStrategy extends BehaviourStrategy{
	...
	private void checkCommunitiesWalls() throws JSONException{
		...
		JSONArray currentCommunitiesIds = new JSONObject(currentCommunities).getJSONObject(RESPONSE_NODE).getJSONArray(ITEMS_NODE);
		...
		for(int i=0; i< currentCommunitiesIds.length(); i++){
			String wallPosts = TextUtils.cp1251ToUTF(apiClient.getWallPosts(COMMUNITY_MAREKER+currentCommunitiesIds.get(i).toString()));
