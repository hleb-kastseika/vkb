...
public class BlackWordsDetector {
	...
	// possible need to add more separator characters
	private static final String WORD_SEPARATORS_REGEX = "\\s*(\\s|=|,)\\s*";
	protected static final String COMMA_SEPARATOR = ",";

	private static PropertiesManager propertyManager = PropertiesManager.getInstance();
	private static DB db = DBMaker.fileDB(new File(propertyManager.getProp(Properties.COMMUNITY_BLACK_LIST_DB_NAME,
			propertyManager.DEFAULT_COMMUNITY_BLACK_LIST_DB_NAME))).closeOnJvmShutdown().make();
	private static Set<String> blackList = db.createHashSet(propertyManager
			.getProp(Properties.COMMUNITY_BLACK_LIST_NAME, propertyManager.DEFAULT_COMMUNITY_BLACK_LIST_NAME))
			.makeOrGet();
