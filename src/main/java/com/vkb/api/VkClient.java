...
import com.vkb.utils.JSONUtils;
...
public class VkClient {
    ...
    private static final String AUTH_URL = "https://oauth.vk.com/authorize" + "?client_id={APP_ID}"
            + "&scope={PERMISSIONS}" + "&redirect_uri={REDIRECT_URI}" + "&display={DISPLAY}" + "&v={API_VERSION}"
            + "&response_type=token";

    private static final String API_REQUEST = "https://api.vk.com/method/{METHOD_NAME}" + "?{PARAMETERS}"
            + "&access_token={ACCESS_TOKEN}" + "&v=" + propertyManager.getProp(Properties.API_VERSION);
    ...
    private void auth(String appId) throws IOException {
        String reqUrl = AUTH_URL.replace("{APP_ID}", appId)
                ...
                .replace("{REDIRECT_URI}", "https://oauth.vk.com/blank.html").replace("{DISPLAY}", "page")
    ...
    public String getAlbums(String userId) {
        String result = "";
        try {
            result = invokeApi("photos.getAlbums",
    ...
    public String getNewsFeed() {
        String result = "";
        try {
            result = invokeApi("newsfeed.get", Params.create().add("count", "100"));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return result;
    ...
    public String getStatus(String userId) {
        String result = "";
        try {
            result = invokeApi("status.get", Params.create().add("user_id", userId));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return result;
    ...
    public boolean isCommunityJoined(String communityId) {
        ...
        try {
            String result = invokeApi("groups.isMember", Params.create().add("group_id", communityId).add("user_id",
                    propertyManager.getProp(Properties.CURRENT_USER_ID)));
    ...
    public String getFriendIds(String userId) {
        String result = "";
        try {
            result = invokeApi("friends.get", Params.create().add("user_id", userId));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return result;
    ...
    public String getWallPosts() {
        String wallPosts = "";
        try {
            wallPosts = invokeApi("wall.get", Params.create().add("count", "100"));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return wallPosts;
    ...
    public String searchNews(String keyWord) {
        String wallPosts = "";
        try {
            wallPosts = invokeApi("newsfeed.search", Params.create().add("q", keyWord).add("count", "200"));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return wallPosts;
    ...
    public String getWallPosts(String owner_id) {
        String wallPosts = "";
        try {
            wallPosts = invokeApi("wall.get", Params.create().add("owner_id", owner_id).add("count", "100"));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return wallPosts;
    ...
    public String doRepost(String objectId) {
        String responce = "";
        try {
            responce = invokeApi("wall.repost", Params.create().add("object", objectId));
    ...
    public boolean putLike(String ownerId, String itemId) {
        ...
        try {
            String responce = invokeApi("likes.add",
                    Params.create().add("owner_id", ownerId).add("item_id", itemId).add("type", "post"));
            if (responce.contains("error") && responce.contains("captcha")) {
                // process captcha
                Logger.error("CAPTCHA!!!!!!!!");
    ...
    public boolean isLiked(String ownerId, String itemId) {
        boolean isLiked = false;
        try {
            String responce = invokeApi("likes.isLiked",
                    Params.create().add("owner_id", ownerId).add("item_id", itemId).add("type", "post"));
            if (responce.contains("error") && responce.contains("captcha")) {
                // process captcha
                Logger.error("CAPTCHA!!!!!!!!");
    ...
    public boolean joinCommunity(String communityId) {
        boolean isJoined = false;
        try {
            String responce = invokeApi("groups.join", Params.create().add("group_id", communityId));
            if (responce.contains("error") && responce.contains("captcha")) {
                // process captcha
                Logger.error("CAPTCHA!!!!!!!!");
    ...
    public String searchCommunities(String searchWord) {
        String responce = "";
        try {
            responce = invokeApi("groups.search", Params.create().add("q", searchWord).add("count", "1000"));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return responce;
    ...
    public String getCommunityById(String communityId) {
        String responce = "";
        try {
            responce = invokeApi("groups.getById", Params.create().add("group_id", communityId));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return responce;
    ...
    public String getUserCommunities(String userId) {
        String responce = "";
        try {
            responce = invokeApi("groups.get", Params.create().add("user_id", userId).add("count", "1000"));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return responce;
    ...
    public String getPrivateMessages() {
        String responce = "";
        try {
            responce = invokeApi("messages.get",
                    Params.create().add("out", "0").add("count", "200").add("time_offset", "0"));
        } catch (IOException e) {
            Logger.error(e, "Error in API invoke");
        }
        return responce;
    ...
    private String invokeApi(String method, Params params) throws IOException {
        ...
        String reqUrl = API_REQUEST.replace("{METHOD_NAME}", method).replace("{ACCESS_TOKEN}", accessToken)
