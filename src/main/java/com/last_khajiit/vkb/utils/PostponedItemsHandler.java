...
public class PostponedItemsHandler {
	...
	private static DB postponedCommunitiesDb = DBMaker.fileDB(new File(propertyManager
			.getProp(Properties.POSTPONED_COMMUNITIES_DB_NAME, propertyManager.DEFAULT_POSTPONED_COMMUNITIES_DB_NAME)))
			.closeOnJvmShutdown().make();
	private static Set<String> postponedCommunities = postponedCommunitiesDb
			.createHashSet(propertyManager.getProp(Properties.POSTPONED_COMMUNITIES_LIST_NAME,
					propertyManager.DEFAULT_POSTPONED_COMMUNITIES_LIST_NAME))
			.makeOrGet();

	private static DB postponedUsersDb = DBMaker.fileDB(new File(propertyManager
			.getProp(Properties.POSTPONED_USERS_DB_NAME, propertyManager.DEFAULT_POSTPONED_USERS_DB_NAME)))
			.closeOnJvmShutdown().make();
	private static Set<String> postponedUsers = postponedUsersDb.createHashSet(propertyManager
			.getProp(Properties.POSTPONED_USERS_LIST_NAME, propertyManager.DEFAULT_POSTPONED_USERS_LIST_NAME))
			.makeOrGet();
